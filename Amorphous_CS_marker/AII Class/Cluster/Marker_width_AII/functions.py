...
def local_marker_DIII(n_orb, L_x, L_y, L_z, x, y, z, P, S, site):
    ...
    marker = (8 * pi / 3) * np.imag(np.trace(M[site * n_orb: site * n_orb + n_orb, site * n_orb: site * n_orb + n_orb]))  # Local marker
    return marker
...
def local_marker_AII(n_orb, L_x, L_y, L_z, x, y, z, P, Q, R, site):
    ...
    Y = np.reshape(Y, (len(Y), 1))                                           # Column vector y
    Z = np.reshape(Z, (len(Z), 1))                                           # Column vector z
    S = np.eye(L_x * L_y * L_z * n_orb) - (2 * R)                            # Chiral symmetry of the path
    ...
    QS = Q @ S  # We calculate them once so that we don't lose time when calculating the invariant
    XQ = X * Q  # We calculate them once so that we don't lose time when calculating the invariant
    YQ = Y * Q  # We calculate them once so that we don't lose time when calculating the invariant
    ZQ = Z * Q  # We calculate them once so that we don't lose time when calculating the invariant

    # From trivial to DIII
    M1 = QS @ XQ @ YQ @ ZQ + QS @ ZQ @ XQ @ YQ + QS @ YQ @ ZQ @ XQ - QS @ XQ @ ZQ @ YQ - QS @ ZQ @ YQ @ XQ - QS @ YQ @ XQ @ ZQ  # Invariant operator
    marker_DIII = (8 * pi / 3) * np.imag(np.trace(M1[site * n_orb: site * n_orb + n_orb,site * n_orb: site * n_orb + n_orb]))  # Local marker

    marker_DIII = np.abs(marker_DIII)


    # Form DIII to AII
    # M2 = (1 / 6) * (-contract(P, Q, P, P) + contract(P, P, Q, P) - contract(P, P, P, Q) + contract(P, Q, Q, Q)
    ...
    #
    # marker_AII = (2 * pi) * np.imag(np.trace(M2[site * n_orb: site * n_orb + n_orb, site * n_orb: site * n_orb + n_orb]))  # Local marker
    #
    marker_AII = 0

    marker = marker_AII + marker_DIII  # Total marker
    # print(" DIII marker=" + str(marker_DIII))
    # print(" AII marker=" + str(marker_AII))
    # print('-------')
