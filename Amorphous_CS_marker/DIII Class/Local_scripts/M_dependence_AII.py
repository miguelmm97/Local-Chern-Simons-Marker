    ...
    for num_rea in range(0, n_realisations):
        ...
        for index in range(0, len(M)):
            ...
            H_periodic, matrix_neighbours = AmorphousHamiltonian3D_WT(n_sites, n_orb, n_neighbours,
                                                            L_x, L_y, L_z, x, y, z, M[index], t1, t2, lamb, "Closed")
            ...
            for i, site in enumerate(lsit_sites):
                marker[i] = local_marker_AII(n_orb, L_x, L_y, L_z, x, y, z, P, Q, R, site)
            marker_average[index] = np.mean(marker)  # Average of the marker for the specified M over sites
            marker_error[index] = np.std(marker) / np.sqrt(sample_sites)  # Average of the marker for the specified M over sites

        marker_final[num_size, :] = marker_final[num_size, :] + marker_average  # Cumulative marker

    marker_final[num_size, :] = marker_final[num_size, :] / n_realisations  # Average of the marker
