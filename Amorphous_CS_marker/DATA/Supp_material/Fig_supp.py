...
for file in os.listdir(outdir):
    if file == "Sup_material2_avmarker.h5":
        with h5py.File(file, 'r') as f:
            datanode = f['data']
            av_marker = datanode[()]

    if file == "Sup_material2_marker.h5":
        with h5py.File(file, 'r') as f:
            datanode = f['data']
            marker = datanode[()]

    if file == "Sup_material2_xaxis.h5":
        with h5py.File(file, 'r') as f:
            datanode = f['data']
            x_axis = datanode[()]

    if file == "Sup_material2_yaxis.h5":
        with h5py.File(file, 'r') as f:
            datanode = f['data']
            y_axis = datanode[()]
...
plt.savefig("Sup_material2.pdf", bbox_inches="tight")
plt.show()
